...
def cache_key(
    *, request_body: dict[str, Any], model_version: str, knowledge_base_version: str
    ...
    return f"company-ai-analysis:{model_version}:{knowledge_base_version}:{body_hash}"


class AiAnalysisCache:
    def __init__(self, path: str | None = None, ttl_seconds: int | None = None) -> None:
        self.path = Path(path or settings.ai_analysis_cache_path)
        self.ttl_seconds = ttl_seconds or settings.ai_analysis_cache_ttl_seconds
        self._ensure_table()
    ...
    def set(self, key: str, response: dict[str, Any]) -> None:
        ...
        with closing(self._connect()) as connection:
            connection.execute(
                """
                insert into ai_analysis_cache(cache_key, response_json, created_at, expires_at)
                values (?, ?, ?, ?)
