...
MODEL_VERSION = "ai-review-opinions-v1"
...
def generate_review_opinions(payload: dict[str, Any]) -> dict[str, Any]:
    budget_line = _budget_mismatch_line(payload)
    employment_line = _employment_carousel_line(payload)
    return {
...
def _budget_mismatch_line(payload: dict[str, Any]) -> str | None:
    ...
    recent_support_text = " ".join(support.get("recentSupportTexts") or [])
    ...
    if not (repeated_support_observed or low_outcome_observed):
        return None
...
def _employment_carousel_line(payload: dict[str, Any]) -> str | None:
    support = payload.get("supportSummary") or {}
